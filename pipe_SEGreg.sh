...
#
# SC segmentation based registration (upper baseline). This approach should represent the best registration results
# that could be obtained as the overlapping of the SC segmentations will be maximized and the registration results
# are assessed by computing the SC segmentations overlapping of the pair of images.
# It consists of a first affine (translation + rotation + scaling) registration step followed by a slice-by-slice
# regularized registration and a deformable registration using mean squares similarity metric.
#
#
# Usage:
# sct_run_batch -jobs 1 -path-data bids_dataset -path-out res_registration -script pipe_SEGreg.sh
#
#
# Author: Evan Béal
...
# Compute SC segmentation of the fixed and moving image to use it for the registration
...
file_mov_seg="${file_mov_before_proc}_seg"

# Perform the segmentation based registration moving --> fixed
# NOTE: We are passing the un-resampled / original moving image as the input to avoid double interpolation
# First step, segmentation based affine registration. Second step, segmentation based slice-by-slice regularized registration. Third step, segmentation based deformable registration.
sct_register_multimodal -i ${file_mov_before_proc}${MOV_EXT} -d ${file_fx_before_proc}_res1.nii.gz -iseg ${file_mov_seg}.nii.gz -dseg ${file_fx_seg}.nii.gz -o ${file_mov_before_proc}_reg.nii.gz -owarp ${file_mov_before_proc}_warp_field.nii.gz -param step=1,type=seg,algo=affine,metric=MeanSquares,smooth=0,iter=30,deformation=1x1x1:step=2,type=seg,algo=slicereg,metric=MeanSquares,smooth=0,iter=30:step=3,type=seg,algo=syn,metric=MeanSquares,smooth=0,iter=30,deformation=1x1x1,shrink=2
