...
def register(model_inference_specs, reg_model1, reg_model2, fx_im_path, mov_im_path, fx_contrast='T1w'):
    """
    Register the moving image to the fixed one using the provided models.
    ...
    if not model_inference_specs['use_subvol']:
        if warp_interp == 'linear':
            ...
            moved, warp_second_reg = model2.predict([np.expand_dims(moved_first_reg.squeeze(), axis=(0, -1)),
                                                     np.expand_dims(fixed.get_fdata().squeeze(), axis=(0, -1))])
            scale = 1 if warp_first_reg[0, ...].shape[0] == model_in_shape[0] else 2
        ...
        # TODO: THE USE OF SUBVOLUMES IS NOT SUPPORTED FOR THE MOMENT
        if warp_interp == 'linear':
            warp_field_lst = []
