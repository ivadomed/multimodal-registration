...
#
# ANTs image based rigid registration + image based SliceReg + ANTs image based deformable registrations focusing
# on the SC using a mask to optimize the similarity metric. This registration pipeline should represent the most
# accurate image based registration results of the SC that can be obtained using iterative methods.
# The mask is obtained by dilating the SC segmentation using a mathematical morphology operation with a ball-shaped
# structuring element of radius 8 voxels. Similar parameters than in IMD and IMRSR are used for the deformable, rigid
# and slice-by-slice regularized registration.
#
#
# Usage:
# sct_run_batch -jobs 1 -path-data bids_dataset -path-out res_registration -script pipe_IMRSRD-priorSC.sh
#
#
# Author: Evan Béal
...
# Compute SC segmentation of the fixed image to focus the registration on the SC
segment ${file_fx_before_proc}_res1 $FX_CONTRAST
file_fx_seg="${file_fx_before_proc}_res1_seg"
# Dilate spinal cord to use it as a mask
sct_maths -i ${file_fx_seg}.nii.gz -dilate 8 -shape ball -o ${file_fx_seg}_dilate.nii.gz
...
# First step, image based rigid registration. Second step, image based slice-by-slice regularized registration. Third step, image based deformable registration.
sct_register_multimodal -i ${file_mov_before_proc}${MOV_EXT} -d ${file_fx_before_proc}_res1.nii.gz -m ${file_fx_seg}_dilate.nii.gz -o ${file_mov_before_proc}_reg.nii.gz -owarp ${file_mov_before_proc}_warp_field.nii.gz -param step=1,type=im,algo=rigid,metric=MI,smooth=0,iter=30,deformation=1x1x1:step=2,type=im,algo=slicereg,metric=MI,smooth=0,iter=30:step=3,type=im,algo=syn,deformation=1x1x1,metric=MI,smooth=0.5,iter=30,shrink=2
