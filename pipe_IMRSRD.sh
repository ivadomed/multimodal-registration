...
#
# ANTs image based rigid registration + image based SliceReg + ANTs image based deformable registration.
# This approach is somewhat similar to what is sought to be achieved with the cascade approach of using
# two deep learning-based deformable registration models in succession, first bringing closer the moving and
# fixed images and then refining the registration.
# Similar parameters than in IMD and IMRSR are used for the deformable, rigid and slice-by-slice regularized registration.
#
#
# Usage:
# sct_run_batch -jobs 1 -path-data bids_dataset -path-out res_registration -script pipe_IMRSRD.sh
#
#
# Author: Evan Béal
