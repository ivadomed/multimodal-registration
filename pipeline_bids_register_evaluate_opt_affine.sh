...
set +e +o pipefail
python $PATH_SCRIPT/eval_reg_on_sc_seg.py --fx-seg-path $file_t1_seg --moving-seg-path $file_t2_seg --warped-seg-path $file_t2_reg_seg --sub-id ${SES} --out-file $PATH_DATA_PROCESSED/metrics_on_sc_seg.csv --append 1 --min-dice $MIN_SC_DICE_EXPECTED_PERC --last-eval 0
RETURN_VAL=$?
set -e -o pipefail
conda deactivate
