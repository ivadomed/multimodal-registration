...
#
# ANTs image based rigid registration + image based (axial) slice-by-slice regularized registration.
# This approach is better adapted than classical affine or rigid methods for the registration of the SC due to
# the articulated nature of the spine. The mutual information similarity metric is used
# for the rigid registration and SliceReg algorithm.
#
#
# Usage:
# sct_run_batch -jobs 1 -path-data bids_dataset -path-out res_registration -script pipe_IMRSR.sh
#
#
# Author: Evan Béal
...
# First step, image based rigid registration. Second step, image based slice-by-slice regularized registration.
sct_register_multimodal -i ${file_mov_before_proc}${MOV_EXT} -d ${file_fx_before_proc}_res1.nii.gz -o ${file_mov_before_proc}_reg.nii.gz -owarp ${file_mov_before_proc}_warp_field.nii.gz -param step=1,type=im,algo=rigid,metric=MI,smooth=0,iter=30,deformation=1x1x1:step=2,type=im,algo=slicereg,metric=MI,smooth=0,iter=30
